...
- `default`
- `default_with_inline_toc`
- `default_with_sidebar_toc`
- `mobeon`
- `mobeon_with_inline_toc`
- `mobeon_with_sidebar_toc`
- `mobeon-dark`
- `mobeon-dark_with_inline_toc`
- `mobeon-dark_with_sidebar_toc`
...
生成物は `exports/<pattern>/` に分かれて保存され、各フォルダ内のファイル名は元 Markdown のベース名をそのまま使います。
...
- `exports/` に生成済みの HTML / PDF から、GitHub Pages 用の `site/` を更新します
- `site/index.html` には `default_with_sidebar_toc` の HTML を使います
- `site/latest.pdf` には `default` の PDF を使います
...
- 共有用の HTML はブラウザ閲覧向けに `default_with_sidebar_toc` を採用します
- 共有用の PDF は配布向けに `default` を採用します

## アーカイブ運用

- 現行の生成物は `exports/<pattern>/` に置きます
...
4. 対象パターンの `exports/<pattern>/` と `site/` を確認する
