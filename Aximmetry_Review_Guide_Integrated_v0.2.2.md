...
- トラッキングワークフローなのに カメラ の前提が曖昧なまま進める
- Aximmetry側で頑張ればシーン側の問題も吸収できると思ってしまう
...
- REtracker Bliss: **あなたの日本語Blissマニュアル（ファイル名をここに記入）** を一次参照にすることを推奨
- REtracker Official Tutorials: [Tutorials](https://www.retracker.co/tutorials)

> Bliss については、英語動画だけで理解を進めるのは日本語話者には負荷が高いため、まずは **あなたの日本語Blissマニュアル（ファイル名をここに記入）** を主参照にする設計が適切です。
...
### 推奨する参照順
1. **あなたの日本語Blissマニュアル（ファイル名をここに記入）**
2. 公式製品ページ
3. 公式チュートリアル
...
> 日本語話者にとっては、Bliss の英語動画だけで再現するのは難しいため、まずは **あなたの日本語Blissマニュアル（ファイル名をここに記入）** を主参照にするのが望ましいです。  
> ファイル名やリンクは、あとでここに差し込んでください。
...
この資料は、学習・復習のためのロードマップです。  
本文の役割を保つため、学習導線と参照導線を中心に整理し、運用メモが必要な場合は別資料として分けて管理します。

- 公式Learnの導線が変わったらリンクを更新する
- **あなたの日本語Blissマニュアル（ファイル名をここに記入）** の実名差し込みを行う
- version 差による UI の違いは本文に詰め込みすぎず、必要なら別表に分ける
- 学習者がつまずきやすい症状例を、Troubleshooting に追記していく
- スクリーンショットを足す場合は「どの画面を見るか」の確認用に絞る
- 初回学習向けの最短ルートと、復習向けの参照ルートを必要に応じて分ける
