...
> また、Aximmetry公式Learnは日本語表示を選べる場合がありますが、**表記やリンク先が安定しないことがある** ため、本資料では暫定的に英語版のリンクを掲載しています。  
> 現状では、ブラウザの Google 翻訳などを併用して読む方が、参照先の対応関係を保ちやすく、理解もしやすいと考えています。
...
6. **再生確認・記録・オフライン確認**: 何を残し、何に使うかを分けて理解する
7. **症状別トラブルシューティング**: 症状から見る場所を絞る
8. **発展学習の入口**: やりたいこと別に、次に開く公式Learnの入口を選ぶ
9. **目的別リンク索引**: 目的から必要な参照先をすぐ引けるようにする
...
- **再生確認 / Playback / Replay**  
  本資料では、現場でのプレイバック確認と、記録した素材をあとで見直す確認の両方を含む言葉として使います。  
  英語の `Playback` と `Replay` は、この資料では同じ系統の言葉として扱います。

- **タイムコード同期 / Timecode Sync**  
  映像、トラッキング、ズームなどの時間軸をそろえるための考え方です。  
  本資料では、Aximmetry内の `Timecode Sync` や `Zoom Timecode Sync` を含む言葉として使います。

- **CV Protocol**  
  Canon カメラから Ethernet 経由でレンズ情報やメタデータを送るための仕組みです。  
  本資料では、ズームやフォーカスなどのレンズデータをAximmetryへ渡す文脈で扱います。

- **billboard**  
  常にカメラ正面を向く表示物や平面オブジェクトのことです。  
  合成用のサインやパネル表現で使われることがあります。

- **Host PC**  
  トラッキング機材や周辺機器が接続される、運用側のPCを指します。  
  本資料では、Aximmetryを動かすPCや、関連デバイスを受ける側のPCという意味で使います。

- **AUTO**  
  入力信号の解像度やフレームレートを自動判別する設定やモードです。  
  うまく判別できない場合は手動指定が必要になることがあります。

- **fps**  
  `frames per second` の略で、1秒あたりのフレーム数を指します。  
  本資料では、映像信号や入力設定のフレームレートを示す言葉として使います。
...
> **補足**  
> 参照先のURLや旧資料では `Unreal Editor for Aximmetry` の名称が残る場合がありますが、本資料では `AX Scene Editor` に統一して扱います。
...
# 6. 再生確認・記録・オフライン確認 / Playback, Recording, and Offline Review
...
### Aximmetryで操作画面やControl Boardを作りたい人へ
...
### 外部コントローラー / トリガー運用を広げたい人へ

**向いている人**

現場で物理ボタンやダイヤルから操作したい人、オペレーター用のトリガー系導線を整えたい人、Stream Deck や Loupedeck、MIDI などを Aximmetry に結びつけたい人。
...
# 9. 目的別リンク索引 / Link Index by Goal

> 何をしたいかから引けるように整理しています。  
> 第8章で学習の方向を決めたあとに、ここでは **必要なリンクをすぐ引く** ために使います。  
...
**Aximmetryで操作画面やControl Boardを作りたい:**
...
> **補足**  
> REtracker Bliss の公式Tutorialには **Vanilla Unreal Engine 向けの内容** が含まれるため、この資料のAximmetry運用の本流では主参照にしていません。  
> 必要に応じて、Unreal Engine 単体運用の参考資料として参照してください。
