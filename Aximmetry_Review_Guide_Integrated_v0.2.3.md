...
> トレーニングを受けた方の復習用としてはもちろん、一部の講義だけ受けた方や、導入説明後に抜けた内容を補いながら確認したい方にも使えるように構成しています。
...
### 1. まず全体像をつかむ
最初に **システム概要 / System Overview** を読み、映像・レンズ情報・トラッキング情報の3系統がどこから来て、どこで統合されるのかを理解します。

### 2. 次に「背景側」の前提を整える
Aximmetryの画面を触る前に、**AX Scene / Unreal準備 / AX Scene / Unreal Setup** を確認します。  
ここでの目的は、Aximmetryに渡す前の **受け皿** を整えることです。

### 3. その後に「同期」の意味を理解する
**システム同期とキャリブレーション / System Synchronization and Calibration** を読み、Tracking Calibration と Nodal Offset が何のために必要なのかを理解します。  
ここを飛ばすと、後段のズレや違和感が「何となく合わない」ままになります。

### 4. 本資料のトラッキングカメラワークフローを通す
**トラッキングカメラワークフロー / Tracked Camera Workflow** から、Aximmetry側の INPUTS / delay / sync / keying / output を順番に復習します。

### 5. つまずいたら症状から引く
途中で詰まったら、**症状別トラブルシューティング / Troubleshooting by Symptom** に戻ります。

---

## このロードマップの見取り図

1. **システム概要 / System Overview**: 3系統の信号と役割分担をつかむ
2. **AX Scene / Unreal準備 / AX Scene / Unreal Setup**: 背景側の受け皿を整える
3. **システム同期とキャリブレーション / System Synchronization and Calibration**: ズレの原因になる同期の意味を理解する
4. **トラッキングカメラワークフロー / Tracked Camera Workflow**: 本資料の主題となる順番で I/O から出力まで追う
5. **キーイングとライブ出力 / Keying and Live Output**: キーイングと当日収録の考え方を押さえる
6. **再生・記録・オフライン確認 / Replay, Recording, and Offline Review**: 何を残し、何に使うかを分けて理解する
7. **症状別トラブルシューティング / Troubleshooting by Symptom**: 症状から見る場所を絞る
8. **実践的な発展項目 / Practical Extensions**: 本流の外側にある発展項目の入口を知る
9. **目的別索引 / Purpose Index**: 目的から参照先を引けるようにする
...
- 本資料で詳しく触れていないもの: AR の詳細、LED Wall ワークフローの詳細、トラッキングなしの Virtual Camera / ヴァーチャルカメラ ワークフローの詳細
...
- **Virtual Camera / ヴァーチャルカメラ**  
  ...
  本資料では詳細は扱わず、Tracked Camera と区別して考えます。

- **Tracked Camera Workflow / トラッキングカメラワークフロー**  
  本資料で主題として扱う、トラッキングカメラを使ったグリーンバック制作の流れです。  
...
- **Tracking Offset / トラッキングオフセット**  
  トラッキングセンサーとカメラ側の基準位置のズレを扱う補正です。  
  手動または専用ソフトウェアで算出することがあります。

- **Lens Calibration / レンズキャリブレーション**  
  レンズの光学特性を計測する工程です。  
  歪みや視点位置の変化などを扱い、チェッカーボードなどと専用ソフトウェアで算出することが多いです。  
  カメラや連携方式によっては、この工程を前提にしない運用もあります。

- **Lens Profile / レンズプロファイル**  
  レンズキャリブレーションの結果をまとめたデータです。  
  ズームやフォーカスに応じた補正に使われます。  
  レンズキャリブレーション済みの環境では、レンズごとにこのプロファイルを切り替えて運用することがあります。

- **Nodal Offset**  
  トラッキングデバイスの基準点と、実際のレンズの視点（瞳入射位置）のズレを補正する工程です。
...
- **Nodal Offset = トラッカーの基準位置と、実際のレンズの視点を一致させる**
...
そのうえでレンズ側の扱いは構成によって異なります。  
Canon C400 では、瞳入射位置を含むレンズ側の補正情報が CV Metadata により更新されるため、通常のレンズキャリブレーション工程を前提にしない運用ができます。  
そのため、ズームやレンズ交換をしてもデータが自動反映され、オペレーターが都度レンズプロファイルを切り替えたり、レンズキャリブレーションを行ったりするプロセスを省くことができます。  
C400 以外でも、レンズキャリブレーション済みであれば、毎回すべてを手動で入れ直すというより、対応するレンズプロファイルを切り替えて運用することが一般的です。

補足:

- Tracking Offset は、トラッキングセンサーとカメラ側の位置関係を扱う補正です
- Lens Calibration / Lens Profile は、レンズの光学特性を計測し、プロファイル化する考え方です
...
本資料の主題である、**トラッキングカメラを使ったグリーンバック制作の流れ** を理解することです。
...
ここでは AR / LED / トラッキングなしの Virtual Camera / ヴァーチャルカメラ の詳細は扱いません。
...
# 8. 実践的な発展項目 / Practical Extensions
